...
class LISFileCollection:
    ...
    standalone_lis_files: list[str] = field(
        default_factory=list
    )  # Vh5205_C-XX.LIS (standalone metadata)
...
class ComplementaryFiles:
    ...
    primary_processed_data: (
        str  # Vh5205_Complementary_Primary-and-processed-data-series.LIS
    )
...
class LISParser:
    ...
    def load_from_zenodo_graph(self, zenodo_graph: dict) -> None:
        ...
        for record_id, record in zenodo_graph.items():
            ...
            # Check for standalone LIS files: Vh5205_C-XX.LIS (no MD-TR/Creep/Loading suffix)
                ...
                if not self._complementary_pattern.match(
                    ...
                    if "standalone_files" not in test_files[test_id]:
                        test_files[test_id]["standalone_files"] = []

                    test_files[test_id]["standalone_files"].append(record_id)
                    if download_url:
                        self._file_urls[record_id] = download_url
                    logger.debug(
        ...
        for test_id, files in test_files.items():
            if all(k in files for k in ["md_tr", "creep", "loading"]):
                standalone_files = files.get("standalone_files", [])
    ...
    async def _fetch_file_content(
        self, checksum: str, max_retries: int = 3, initial_delay: float = 1.0
    ) -> Optional[str]:
    ...
    def _parse_date(
        self, date_str: str, line_num: int, parse_errors: list, test_id: str
    ) -> Optional[datetime]:
            ...
            try:
                ...
                if "Metadata --> Test info --> Test job details" in categorization:
                    ...
                    if "Material Identifier" in entry:
                        ...
                        "Manufacturing process description as-tested material"
                        in categorization
                    ):
