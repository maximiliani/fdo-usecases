...
async def main():
    """Execute main workflow with proper exit codes.
    ...
    async with LISParser() as lis_parser:
        ...
        # Count Grant FDOs
        grant_fdo_count = sum(
            1 for k in graph_dict.keys() if k.startswith(placeholder_pid("grant:"))
        )
        ...
        failed_test_ids = set(
            e.test_id for e in lis_parser.errors if e.test_id != "unknown"
        )
        successful_tests = all_test_ids - failed_test_ids

        print(
            f"  Successfully parsed: {len(successful_tests)} / {len(all_test_ids)} tests"
        )
        ...
        # Step 6: Compare and sync the graph with Elasticsearch / Typed PID Maker
        es_base_url = os.environ.get("FDO_ES_BASE_URL", "")
        es_index = os.environ.get("FDO_ES_INDEX", "")
        if es_base_url and es_index:
            dry_run = os.environ.get("FDO_SYNC_DRYRUN", "") == "1"
            logger.info(
                "Syncing graph with Elasticsearch index '%s'%s...",
                es_index,
                " (DRY-RUN, Typed PID Maker will not be contacted)" if dry_run else "",
            )
            try:
                await run_sync(
                    ...
                    tpm_host=os.environ.get("FDO_TPM_HOST", DEFAULT_TPM_HOST),
                    es_api_key=os.environ.get("FDO_ES_API_KEY"),
                    es_username=os.environ.get("FDO_ES_USERNAME"),
                    es_password=os.environ.get("FDO_ES_PASSWORD"),
                    tpm_api_key=os.environ.get("FDO_TPM_API_KEY"),
                    dry_run=dry_run,
                )
            except Exception as exc:
                logger.warning(f"Skipping ES/TPM sync due to error: {exc}")
                print(f"\n⚠️  Skipping ES/TPM sync: {exc}")
        else:
            logger.info("ES/TPM sync disabled (set FDO_ES_BASE_URL and FDO_ES_INDEX)")
