...
class MaterialDesign(RecordDesign):
    ...
    def __init__(self, orchestrator: "CreepFDOOrchestrator | None" = None):
        """Initialize material design with orchestrator reference."""
        super().__init__()
    ...
    async def create_fdo(self, data: MaterialData) -> str:
        ...
        # Profiles
        record.addAttribute(PROFILE_KEY, [MATERIAL_PROFILE, BASE_PROFILE])
        ...
        for file_pid in data.referenced_files:
            ...
            existing_links = [
                ...
                if attr[0]
                in [INFOTYPES["hasChemicalComposition"], INFOTYPES["hasHeatTreatment"]]
            ]
        ...
        record.addAttribute(INFOTYPES["name"], f"Material {data.material_id}")
        ...
        logger.debug(
            f"Created Material FDO for {data.material_id} ({data.pid_composition})"
        )
