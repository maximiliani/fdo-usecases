...
class CreepExperimentDesign(RecordDesign):
    ...
    async def create_fdo(self, data: CreepExperimentData) -> str:
        ...
        # Profiles
        record.addAttribute(
            PROFILE_KEY, [CREEP_EXPERIMENT_PROFILE, BASE_PROFILE, VERSIONABLE_PROFILE]
        )
        ...
        if data.percentage_creep_extension > 0:
            record.addAttribute(
                INFOTYPES["percentageCreepExtension"], data.percentage_creep_extension
            )

        # Only add if non-zero (soft fail for missing data)
        if data.single_crystal_orientation > 0:
            record.addAttribute(
                INFOTYPES["SingleCrystalOrientation"], data.single_crystal_orientation
            )
        ...
        record.addAttribute(INFOTYPES["name"], f"Creep Test {data.test_id}")
        record.addAttribute(
            INFOTYPES["dateCreated"], data.test_job_details["dateOfTestStart"]
        )

        # Description
        description = (
            f"Creep test at {data.specified_temperature}°C, {data.initial_stress} MPa"
        )
