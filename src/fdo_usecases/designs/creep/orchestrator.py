...
class CreepFDOOrchestrator(RecordDesign):
    ...
    async def execute_async(self, lis_parser: "LISParser") -> None:
        ...
        sem_images, preview_image_urls = lis_parser.extract_images_from_graph(
            self._record_graph
        )

        # Get creators/affiliations from Dataset
        dataset_creators = lis_parser.get_creators_from_dataset(self._record_graph)
        dataset_affiliations = lis_parser.get_creator_affiliations_from_dataset(
            self._record_graph
        )
        ...
        if common_metadata:
            logger.info(
                f"Common metadata: Material={common_metadata.material_id}, Standard={common_metadata.applicable_standard}"
            )
        ...
        async def parse_test(
            test_id: str, collection: "LISFileCollection"
        ) -> tuple[str, Optional["ParsedTestMetadata"]]:
            ...
            if metadata and common_metadata:
                if (
                    not metadata.applicable_standard
                    and common_metadata.applicable_standard
                ):
        ...
        parse_tasks = [
            parse_test(test_id, collection)
            for test_id, collection in file_collections.items()
        ]
        ...
        unique_materials = set(
            m.material_id for m in test_metadata_map.values() if m.material_id
        )
        ...
        for material_id in unique_materials:
            ...
            rep_metadata = next(
                m for m in test_metadata_map.values() if m.material_id == material_id
            )
            ...
            if rep_metadata.manufacturing_as_manufactured:
                descriptions.append(
                    f"As-manufactured: {rep_metadata.manufacturing_as_manufactured}"
                )
            if rep_metadata.manufacturing_as_tested:
                descriptions.append(
                    f"As-tested: {rep_metadata.manufacturing_as_tested}"
                )
            ...
            material_data = MaterialData(
                ...
                pid_composition=material_pid_map[material_id],
    ...
    async def _resolve_file_reference(self, filename: str, context: str) -> str | None:
        ...
        for record_id, record in self._record_graph.items():
            ...
            name_attrs = [
                attr[1] for attr in record._tuples if attr[0] == INFOTYPES["name"]
            ]
            if name_attrs:
